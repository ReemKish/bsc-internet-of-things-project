...
// Definitions for the fall detection
LSM6DSO myIMU; //Default constructor is I2C, addr 0x6B

const int frequency = 100; //in hertz
int total_time = 1000; //in ms 
int time_unit = total_time/frequency; //one cycle duration in ms
float vector_sum;
...
{
  vector_sum = sqrt(pow(myIMU.readFloatAccelX(),2) + pow(myIMU.readFloatAccelY(),2) + pow(myIMU.readFloatAccelZ(),2));
  sliding_window[window_index] = vector_sum;
  window_index ++;
  window_index = window_index % frequency;
  delay(time_unit);
}

bool is_fall()
{
  int cnt_under_threshold_seq = 0;
  for (int i = 0; i < frequency; i++)
  {
    if (cnt_under_threshold_seq >= 5)
    {
      return true;
    }
    if (sliding_window[i] >= threshold)
    {
      cnt_under_threshold_seq = 0;
      continue;
    }
    cnt_under_threshold_seq ++;
  }
...
void loop()
{
  ...
  if (is_fall())
  {
    if (fall_was_reported == 0)
    {
      Serial.print("There is a fall\n");
      sendTelemetry();
    }
    fall_was_reported = 1;
  }
